function GithubCalendar(git_githubapiurl, git_color) {
  debugger;
  if (document.getElementById('github_container')) {
    var github_canlendar = (git_githubapiurl, git_color) => {
      var git_fixed = 'fixed';
      var git_px = 'px';
      ...
      fetch(git_githubapiurl).then(data => data.json()).then(data => {
        ...
        addweek(data);
        addlastmonth();
        var html = github_main_box(git_monthchange, git_data, git_color, git_total, git_thisweekdatacore, git_weekdatacore, git_oneyearbeforeday, git_thisday, git_aweekago, git_amonthago);
